...
const Item = ({name, image, price} : ItemProps) => (
  <ProdutoCard>
    ...
    <AddToCart><ButtonText>Comprar</ButtonText></AddToCart>
  </ProdutoCard>
);
...
const Produtos = () => {
  ...
  return (
    <>
      <Header>
        <ButtonMenu>
          <Entypo name="menu" size={40} color="#705A54" />
        </ButtonMenu>
        <HeaderText>TSpresso</HeaderText>
        <ButtonBasket>
          <Entypo name="shopping-basket" size={35} color="#705A54" />
        </ButtonBasket>
      </Header>

      <Container>
        <Input value="" placeholder="Barra de pesquisa teehee" onChangeText={setBusca}/>
      ...
      {/* <ContainerProdutos> */}
        <FlatList 
        data={aaa}
        renderItem={({item}) => <Item name={item.title} image={item.image} price={item.price}/>} 
        keyExtractor={(item) => item.id}  
        numColumns={2}
        columnWrapperStyle={{gap:10, justifyContent: "center"}}
        showsVerticalScrollIndicator={false}
        contentContainerStyle={{ width: "100%", paddingLeft: 15, paddingRight: 15}}
        />
        {/* </ContainerProdutos> */}
      </Container>
    </>
